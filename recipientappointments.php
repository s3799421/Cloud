...
<?php 
include('config.php');
$sql = "SELECT name, gender, dob, type, doa, time, no, address FROM Recipient_Appointment";
$result = $db->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<br>". $row["name"]. " ; ". $row["gender"]. " ; " . $row["dob"] . " ; " . $row["type"] . " ; " . $row["doa"] . " ; " . $row["time"] . " ; " . $row["no"] . " ; " . $row["address"] . "<br>" ;
    }
} else {
    echo "0 results";
}
